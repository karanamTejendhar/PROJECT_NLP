import re
from english import rough_orig_parser
import load
import regex as reg
...
# from ..parser import is_quote_artifact , get_unit_from_dimensions
...
def clean_surface(surface, span):
    ...
    if (
            split[0] in reg.miscnum()
            and len(split) > 1
            and split[1] in reg.units() + reg.tens()
    ):
            ...
            for word in surface.lower().split():
                try:
                    scale, increment = (
                        ...
                        float(
                            re.sub(
                                r"(-$|[%s])" % reg.grouping_operators_regex(),
                                "",
                                word,
                            )
                        ),
                    ...
                    scale, increment = reg.numberwords()[match.group(0)]
...
def build_quantity(orig_text, text, item, values, unit, surface, span, uncert):
    ...
    if unit.entity.dimensions:
        if (
                ...
                and unit.original_dimensions[1]["surface"] in reg.suffixes().keys()
        ):
        ...
        elif unit.original_dimensions[0]["surface"] in reg.suffixes().keys():
            # k/M etc is only applied if non-symbolic surfaces of other units
            # (because colloquial) or currency units
            symbolic = all(
                dim["surface"] in load.units().names[dim["base"]].symbols
    ...
    while pruned_common_word:
        ...
        for start in range(0, len(unit.original_dimensions)):
            for end in reversed(range(start + 2, len(unit.original_dimensions) + 1)):
                ...
                if combination.lower() not in lang_load.COMMON_WORDS[len(combination)]:
                    continue
    ...
    match = rough_orig_parser.is_quote_artifact(text, item.span())
    if match:
        surface = surface[:-1]
        span = (span[0], span[1] - 1)
    ...
    if dimension_change:
        if unit.original_dimensions:
            unit = rough_orig_parser.get_unit_from_dimensions(
                unit.original_dimensions, orig_text
            )
        else:
...
def parse_unit(_, unit, slash):
    ...
    power_written = re.findall(r"\b(%s)\b" % "|".join(reg.powers()), surface)
    ...
    if power:
        ...
        exponent = reg.powers()[power_written[0]]
